...
jira = jira = JIRA(basic_auth=('e.barnaev', 'Rfqhfnrth#3'), options = {'server': 'https://jira.i.altatec.ru', 'verify':False})
def Get_Currnet_Month_Updated_Issues():
    base_request = 'updated >= startOfMonth() and project = DGPBUS'
...
def Get_Previous_Month_Updated_Issues():
    base_request = 'updated >= startOfMonth(-1)  and updated <= startOfMonth() and project = DGPBUS'
...
def Get_Today_Updated_Issues():
    base_request = 'updated >= startOfDay() and project = DGPBUS'
...
def Get_Yesterday_Updated_Issues():
    base_request = 'updated >= startOfDay(-1) and updated <= startOfDay() and project = DGPBUS'

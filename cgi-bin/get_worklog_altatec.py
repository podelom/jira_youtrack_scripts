...
def Get_Today_Logged_Work():
    for issue in get_issues_updated_altatec.Get_Today_Updated_Issues():
        try:
...
def Get_Yesterday_Logged_Work():
    for issue in get_issues_updated_altatec.Get_Currnet_Month_Updated_Issues():
        try:
            for item in jira.worklogs(issue):
                worklog = jira.worklog(issue, item)
...
def Get_Start_Of_The_Previous_Month_Date():
    for issue in get_issues_updated_altatec.Get_Previous_Month_Updated_Issues():
        try:
            for item in jira.worklogs(issue):
                worklog = jira.worklog(issue, item)
